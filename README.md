...
​	当前系统主要分为两个模块,分别是日志数据导入Hbase(flink-2-hbase)和用户推荐模块(web)
 ...
 - 在日志数据模块(flink-2-hbase)中,又主要分为三个部分:
...
- **2.3 两种推荐逻辑的组合**
  ...
  首先,在基于用户画像的基础上按照热度排序商品,对于排好序的每个商品,接着对每个推荐列表里的产品做相似度关联,选出更多的产品推送给用户,其中这部分产品是**用户未曾点击过**的.
